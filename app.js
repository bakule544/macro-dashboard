...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchJsonWithTimeout(url, label, timeoutMs = 4000) {
  ...
  function setupTradingViewSubTabs() {
    ...
    let singleDrawingMode = true; // TradingView mode: 1 single drawing panel on active targeted chart

    // Helper: Select target box focus
    function setTargetBox(idx) {
      const prevIdx = selectedBoxIndex;
      ...
      // When active chart changes in single drawing panel mode, switch drawing toolbar to targeted chart
      if (singleDrawingMode && prevIdx !== idx) {
        const count = parseInt(localStorage.getItem("activeTvChartLayout") || "1", 10);
        if (count > 1) {
          loadChartIframe(prevIdx + 1);
          loadChartIframe(idx + 1);
        }
      }
    }

    // Auto-detect active chart box on hover, click, or iframe focus (TradingView auto-target behavior)
    chartBoxes.forEach((box, idx) => {
      if (!box) return;
      
      // Auto-target when mouse hovers over any chart panel
      box.addEventListener("mouseenter", () => {
        setTargetBox(idx);
      });
      
      // Auto-target when touched/clicked
    ...
    // Detect direct iframe clicks via window blur focus tracking
    ...
    // Helper: On-demand lazy chart iframe loader with query timestamp update to force reload
    function loadChartIframe(iframeIndex, symbolOverride, intervalOverride) {
      ...
      const symbol = symbolOverride || iframe.getAttribute("data-current-symbol") || iframe.getAttribute("data-default-symbol") || "FOREXCOM:SPXUSD";
      const interval = intervalOverride || iframe.getAttribute("data-current-interval") || "D";
      ...
      const previousHideSide = iframe.getAttribute("data-current-hideside");
      ...
      // Determine drawing panel visibility: ONLY the active targeted chart box gets drawing panel in singleDrawingMode
      const isTargetedBox = (iframeIndex - 1 === selectedBoxIndex);
      const hideSideToolbar = singleDrawingMode ? !isTargetedBox : false;
      const hideSideStr = hideSideToolbar ? "true" : "false";
      iframe.setAttribute("data-current-hideside", hideSideStr);
      ...
      const config = {
        ...
        calendar: false,
        hide_top_toolbar: true,     // Removes internal duplicate top search/TF toolbar inside iframe for ultra-clean look
        hide_legend: true,          // Hides bulky OHLC legend text (Euro/U.S. Dollar - 1D - FXCM O 1.15284...) as requested!
        hide_side_toolbar: hideSideToolbar, // Single drawing panel only on active targeted chart!
        support_host: "https://www.tradingview.com"
      };

      // Query param v=Timestamp ensures browser reloads iframe when configuration changes
      const ts = Date.now();
      const newSrc = `https://s.tradingview.com/embed-widget/advanced-chart/?locale=en&v=${ts}#${encodeURIComponent(JSON.stringify(config))}`;
      
      if (previousSymbol !== symbol || previousInterval !== interval || previousHideSide !== hideSideStr || iframe.src === "about:blank" || !iframe.src) {
        iframe.src = newSrc;
      }
    }
    ...
    // Single Drawing Panel Mode Switcher
    const drawingModeBtn = document.getElementById("tv-drawing-mode-btn");
    if (drawingModeBtn) {
      drawingModeBtn.addEventListener("click", () => {
        singleDrawingMode = !singleDrawingMode;
        if (singleDrawingMode) {
          drawingModeBtn.classList.add("active");
          drawingModeBtn.style.background = "rgba(99,102,241,0.2)";
          drawingModeBtn.style.borderColor = "rgba(99,102,241,0.5)";
          drawingModeBtn.style.color = "#a5b4fc";
          drawingModeBtn.innerHTML = '<i class="fas fa-pen-nib"></i> <span>Drawings: Active Chart</span>';
        } else {
          drawingModeBtn.classList.remove("active");
          drawingModeBtn.style.background = "rgba(255,255,255,0.04)";
          drawingModeBtn.style.borderColor = "rgba(255,255,255,0.08)";
          drawingModeBtn.style.color = "#cbd5e1";
          drawingModeBtn.innerHTML = '<i class="fas fa-pen-nib"></i> <span>Drawings: All Charts</span>';
        }
        const count = parseInt(localStorage.getItem("activeTvChartLayout") || "1", 10);

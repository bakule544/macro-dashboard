...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchJsonWithTimeout(url, label, timeoutMs = 6000) {
    ...
    try {
      ...
      try {
        ...
        // If proxy route returns HTML 404 (e.g. static hosting on GitHub Pages), fallback to direct FRED API!
        if (url.includes("/api/fred/observations") && !url.includes("api.stlouisfed.org")) {
          const directUrl = url.replace(/.*\/api\/fred\/observations\?/, "https://api.stlouisfed.org/fred/series/observations?");
          console.warn(`Proxy endpoint returned non-JSON/404, falling back to direct FRED API: ${directUrl}`);
      ...
      if (!response.ok) {
        if ((response.status === 404 || response.status === 502) && url.includes("/api/fred/observations") && !url.includes("api.stlouisfed.org")) {
          const directUrl = url.replace(/.*\/api\/fred\/observations\?/, "https://api.stlouisfed.org/fred/series/observations?");
          console.warn(`Proxy endpoint returned HTTP ${response.status}, falling back to direct FRED API: ${directUrl}`);
          return await fetchJsonWithTimeout(directUrl, label, timeoutMs);
        }
  ...
  function setupTradingViewSubTabs() {
    ...
    // Auto-detect active chart box on hover, click, or iframe focus (pure CSS highlight, no reloading)
    ...
    function loadChartIframe(iframeIndex, symbolOverride, intervalOverride) {
      ...
      const config = {
        ...
        hide_side_toolbar: false,    // Enables drawing tools & scale options
      ...
      const newSrc = `https://s.tradingview.com/embed-widget/advanced-chart/?locale=en#${encodeURIComponent(JSON.stringify(config))}`;
      
      // STABLE RELOAD: Only update iframe src if symbol or timeframe changed or iframe is uninitialized!
      if (previousSymbol !== symbol || previousInterval !== interval || iframe.src === "about:blank" || !iframe.src || iframe.src.indexOf("advanced-chart") === -1) {
        iframe.src = newSrc;
      }
    }

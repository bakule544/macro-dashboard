...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function logVisitorLocationClientSide() {
    fetch("https://ipapi.co/json/")
      .then(res => res.json())
      .then(data => {
        if (data && data.ip) {
          console.log(`[Visitor Analytics] IP: ${data.ip} | Location: ${data.city}, ${data.region}, ${data.country_name} (${data.org})`);
        }
      })
      .catch(() => {});
  }
  ...
  function renderChartRoomSelector() {
    if (!elements.chartSelectorList) return;
  ...
  async function fetchJsonWithTimeout(url, label, timeoutMs = 6000) {
    ...
    // Serve from instant RAM cache if available and < 5 mins old
    if (RAM_CACHE.has(cacheKey)) {
      const cachedEntry = RAM_CACHE.get(cacheKey);
    ...
    try {
      ...
      try {
        ...
        if (url.includes("/api/fred/observations") && !url.includes("corsproxy") && !url.includes("allorigins")) {
          ...
          console.warn(`Proxy endpoint returned non-JSON/404, attempting CORS proxy: ${corsUrl}`);
          return await fetchJsonWithTimeout(corsUrl, label, timeoutMs);
        } else if (url.includes("corsproxy.io")) {
          const directUrl = decodeURIComponent(url.replace("https://corsproxy.io/?", ""));
          const fallbackUrl = `https://api.allorigins.win/raw?url=${encodeURIComponent(directUrl)}`;
          console.warn(`Corsproxy returned non-JSON, attempting secondary proxy: ${fallbackUrl}`);
          return await fetchJsonWithTimeout(fallbackUrl, label, timeoutMs);
        }
      ...
      if (!response.ok) {
        if ((response.status === 404 || response.status === 502) && url.includes("/api/fred/observations") && !url.includes("corsproxy") && !url.includes("allorigins")) {
          const directUrl = url.replace(/.*\/api\/fred\/observations\?/, "https://api.stlouisfed.org/fred/series/observations?");
          const corsUrl = `https://corsproxy.io/?${encodeURIComponent(directUrl)}`;
          console.warn(`Proxy endpoint returned HTTP ${response.status}, attempting CORS proxy: ${corsUrl}`);
          return await fetchJsonWithTimeout(corsUrl, label, timeoutMs);
        }
      ...
      if (data.error_message) {
        if (RAM_CACHE.has(cacheKey)) return RAM_CACHE.get(cacheKey).data;
      ...
      // Save to RAM Cache
      RAM_CACHE.set(cacheKey, { timestamp: Date.now(), data });
      return data;
    } catch (err) {
      if (url.includes("/api/fred/observations") && !url.includes("corsproxy") && !url.includes("allorigins")) {
        const directUrl = url.replace(/.*\/api\/fred\/observations\?/, "https://api.stlouisfed.org/fred/series/observations?");
        const corsUrl = `https://corsproxy.io/?${encodeURIComponent(directUrl)}`;
        console.warn(`Proxy fetch failed (${err.message}), falling back to CORS proxy: ${corsUrl}`);
        return await fetchJsonWithTimeout(corsUrl, label, timeoutMs);
      } else if (url.includes("corsproxy.io")) {
        const directUrl = decodeURIComponent(url.replace("https://corsproxy.io/?", ""));
        const fallbackUrl = `https://api.allorigins.win/raw?url=${encodeURIComponent(directUrl)}`;
        console.warn(`Corsproxy fetch failed (${err.message}), falling back to secondary proxy: ${fallbackUrl}`);
        return await fetchJsonWithTimeout(fallbackUrl, label, timeoutMs);
      }

      if (RAM_CACHE.has(cacheKey)) {
        console.warn(`Fetch error (${err.message}), returning cached data for ${label}`);
        return RAM_CACHE.get(cacheKey).data;
      }
